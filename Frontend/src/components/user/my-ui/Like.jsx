...
const Like = ({ className, productId }) => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            // Suppress error toast for guests
            if (user) {
        ...
        fetchCounts();
    }, [actualProductId, user]);

    const handleLike = async () => {
                if (!user) {
                        // Guests cannot react; keep UI passive without noisy toasts
                        return;
                }
        if (!actualProductId) {
            console.error("No product ID available");
            ...
            <div className="like-dislike-container">
                ...
                <div className="icons-box">
                    <div className="icons">
                        ...
                        <span className="like-text-content" title={!user ? 'Login to react' : undefined} style={{ color: isLike ? '#4CAF50' : '#666' }}>{likeCount}</span>
                        </label>
                    </div>
                    ...
                    <div className="icons">
                        ...
                        <span className="dislike-text-content" title={!user ? 'Login to react' : undefined} style={{ color: isDislike ? '#F44336' : '#666' }}>{dislikeCount}</span>
                        </label>
                    </div>
                </div>
            </div>
